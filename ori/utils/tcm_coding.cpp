...
long_matrice back_ward_treillis(long_matrice FW_treillis)
{
	long_matrice BW;
	long i,j,k;
	BW=init_long_mat(FW_treillis.lin,FW_treillis.col,0);
	for (j=0; j<BW.col ; j++)
		for (k=0; k<BW.lin;k++)
			for(i=0; i<BW.lin; i++)
...
{
	long i,N,L,j,ind_min,min;
	long_vector tail;
	N=(long)floor(log10(trans.col)/log10(2.0)+0.5); // nombre de bits à l'entrée du codeur
	L=(long)floor(log10(trans.lin)/log10(2.0)+0.5); // nombre de registres
	tail=init_long_vect(L-N+1,0);
...
{
	long m,max_state,max_in,i,mL,j,sym_in,state,next_state, LC,pos;
	long *p_y, *p_nx,*p_x;
	...
	// max_in=2^m
	LC=(long)floor(log10(trans.lin)/log10(2.0)+0.5)+1; // Longueur de contreinte

	y=init_long_vect(x.taille/m+(fermeture!=0)*(LC-m),0); //pour ajouter eventuellement les bits de tail
...
cmplx_vect mapper_8PSK(long_vector x)
{
	long i;
	cmplx_vect y;
	y=init_cmplx_vect(x.taille,init_cmplx(0,0));
...
cmplx_vect scat_8PSK()
{
	long i;
	cmplx_vect scat;
	scat=init_cmplx_vect(8,init_cmplx(0,0));
	double re[8]={1.0,0.7071,0.0,-0.7071,-1.0,-0.7071,0.0,0.7071};
	double im[8]={0.0,0.7071,1.0,0.7071,0.0,-0.7071,-1.0,-0.7071};
	for (i=0; i<8; i++){
		scat.vect[i].Re=re[i];
...
*/
long_vector symbol_interleaver(long_vector in, long_vector intrl,long m)
{
	long i,j;
	long_vector out;
	out=init_long_vect(in.taille,0);
...
*/
long_vector symbol_deinterleaver(long_vector in, long_vector intrl, long m)
{
	long i,j;
	long_vector out;
	out=init_long_vect(in.taille,0);
...
cmplx_vect symbol_deinterleaver(cmplx_vect in,long_vector intrl,long m)
{
	long i,j;
	cmplx_vect out;
	out=init_cmplx_vect(in.taille,init_cmplx(0,0));
	for (i=0; i<intrl.taille; i++)
		for (j=0; j<m; j++)
			out.vect[m*i]=in.vect[intrl.vect[i]*m+j];
	return out;
					  ...
					  cmplx_vect scat, long close_switch)
{
	...
	M=trans.lin; // nombre des etats dans le treillis
	N=trans.col; // nombre de symboles d'information
	inf=1e6;
	metric_cum=init_matrice(M,size_total,inf);
	last_state=init_long_mat(M,size_total,0);
	...
	for (t=size_total-2; t>=0; t--){
		for (etat_dep=0; etat_dep<Ms; etat_dep++){
			for(som=-inf,j=0;j<N;j++){ // j est l'entrée du codeur (symbole d'info)
				...
				if(modul2(x.vect[t])==0) gama=app.mat[j][t];
				else gama=app.mat[j][t]-
					modul2(cmplx_sub(x.vect[t],scat.vect[output.mat[etat_dep][j]]))/2/sigma2;
...
for (j=0; j<beta.lin; j++)
fprintf(fp,"%lf\t",beta.mat[j][i]);
fprintf(fp,"\n");
}
fprintf(fp,"\n-----------------------------\n\n");*/
	//Calcu de la matrice de alpha
	alpha=init_matrice(Ms,size_total,-inf);
	alpha.mat[0][0]=0;
	for (t=1; t<size_total; t++){
		for (etat_arr=0; etat_arr<Ms; etat_arr++){
			for(som=-inf,j=0;j<N;j++){ // j est l'entrée du codeur (symbole d'info)
				...
				if(modul2(x.vect[t-1])==0) gama=app.mat[j][t-1];
				else gama=app.mat[j][t-1]-
					modul2(cmplx_sub(x.vect[t-1],scat.vect[output.mat[etat_dep][j]]))/2/sigma2;
	...
	for (t=0;t<x.taille;t++){
		for (entree=0; entree<N; entree++){ // collone entree et ligne i de la matrice trans est l'etat suivant pour l'entrée du codeur égale à entree
			for (som=-inf,etat=0 ; etat<Ms ; etat++){ // pour tous les etats
				...
				if(modul2(x.vect[t])==0) gama=app.mat[entree][t];
				else gama=app.mat[entree][t]-
					modul2(cmplx_sub(x.vect[t],scat.vect[output.mat[etat][entree]]))/2/sigma2;
...
{
FILE *fp;
	long Ms,N,m,pos,pos_MSB,i,j,ind_max;
	double max;
	long_vector sym_dec;
	long *p_bit_dec;
	sym_dec=init_long_vect(r.taille,0);
	Ms=trans.lin; // nombre d'états dans le treillis
	N=trans.col; // nombre de symboles d'information (2^k)
	m=(long)floor(log10(N)/log10(2.0)+0.5); // nombre de bits à l'entrée du codeur
	matrice LLR=init_matrice(N,r.taille,0);
	cmplx_vect rec1, rec2;
