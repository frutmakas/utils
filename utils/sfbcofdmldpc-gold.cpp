 ...
 * $Name:  $
 * $Author: syed $
 * $Date: 2004/03/24 20:44:22 $
 * $Revision: 1.1.2.10 $
 * $Id: sfbcofdmldpc.cpp,v 1.1.2.10 2004/03/24 20:44:22 syed Exp $
 *******************************************************************/
...
int main() {
	const int nb_user=4, nb_user_max=7, nb_porteuse=128, nb_frame=50; 
	const int snr_min=0, snr_max=25, snr_step=3;
	const int interleaver_size=128000;
...
#endif
for(filter_length=4;filter_length<6;filter_length+=2) {
    int frame_size=5;
	...
	logfile<< "cdma code used : gold (default initialvalue and polynomial)" <<endl;
#ifndef DONT_USE_INTERLEAVER
	...
	char varname[5000];
	int u,k,l;
	DMatrix gold(nb_user_max, nb_user_max);
	DVector dtmp;
...
//		dtmp = dWalshCode(nb_user_max, u);
        dtmp = dGoldCode(InitialValue, Polynomial, 3, 3, u);
        logfile << "user " << u << " spreading sequence :-" << dtmp;
		for(int l=0;l<nb_user_max;l++) {
			gold.mat[u][l]=dtmp.vect[l];
	...
	DMatrix goldspreadcoef = gold.transpose_nip()*(gold*gold.transpose_nip()).inv_nip();
	logfile<< "ouch .. !" << endl;
...
//				ZMatrix y1 = nrx1*walsh.transpose_nip();
				ZMatrix y1 = nrx1*goldspreadcoef;
...
//			sptr=sptr+nb_porteuse;        
		...
		for(u=0;u<nb_user;u++) {        dataifile[u].close(); };
