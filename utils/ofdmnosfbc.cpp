...
int main() {
	const int nb_user=1, nb_user_max=2, nb_porteuse=128, nb_frame=100; 
...
/*					for (k=1;k<nb_porteuse;k+=2) {
	...
	*/				ZMatrix z(2,1);
					...
					for (k=0;k<nb_porteuse;k+=1) {
						z.mat[0][0] = L.vect[u].vect[0].mat[k][k].conj()*zy.mat[k][0];//+
//									  L.vect[u].vect[1].mat[k][k]*zy.mat[k+1][0];
		/*				z.mat[1][0] = L.vect[u].vect[0].mat[k][k]*zy.mat[k+1][0]-
						...
						DCplx Lh = L.vect[u].vect[0].mat[k][k].conj()*L.vect[u].vect[0].mat[k][k];//+
