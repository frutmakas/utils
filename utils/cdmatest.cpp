...
#define const_values
	...
	for(i=0;i<nb_user_max;i++) {
		userbits.vect[i]=WVector(bitsize,1);
		bRandBit1(&bseed, userbits.vect[i]);
		//cout << userbits.vect[i] << endl;
	...
	for(int snr=0;snr<42;snr+=3) {
		...
		for(i=0;i<nb_user_max;i++) {
			...
			//cout << bitrx.taille << " " << userbits.vect[i]<<endl;
