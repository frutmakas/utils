 ...
 * $Name:  $
 * $Author: syed $
 * $Date: 2004/01/16 07:46:36 $
 * $Revision: 1.1.2.13 $
 * $Id: MIMOStbc.cpp,v 1.1.2.13 2004/01/16 07:46:36 syed Exp $
 ********************************************************************/
...
#define DECOMPOSED        0
#define __CVS_REVISION__ "$Id: MIMOStbc.cpp,v 1.1.2.13 2004/01/16 07:46:36 syed Exp $"
...
UVector<ZVector> propagateASTBCInFadingChannel(const ZMatrix &X, int nb_rx, int nb_user, double Es, double SNR, UMatrix<ZMatrix> &H, dRandUniStatePtr *tapseed) {
	...
	zRandGausInit(&noise_ptr ,0, convertsnrtosigma2(0.5*Es/(double)(nb_user), SNR));
...
UVector<ZVector> propagateASTBCInFadingChannelWithoutNoise(const ZMatrix &X, int nb_rx,  UMatrix<ZMatrix> &H, dRandUniStatePtr *tapseed) {
	H = UMatrix<ZMatrix>(2, nb_rx);
	UVector<ZVector> r(nb_rx);
	double doppler = 10, symbolT_us =1;
	int blocksize = X.line;
	...
	for(i=0;i<nb_rx; i++) {
		r.vect[i] = ZVector(blocksize);
		for(j=0, frame=0; j<blocksize; j+=2, frame++) {
			r.vect[i].vect[j]=H.mat[0][i].mat[frame][0]*X.mat[j][0]+H.mat[1][i].mat[frame][0]*X.mat[j][1];
			r.vect[i].vect[j+1]=H.mat[0][i].mat[frame][0]*X.mat[j+1][0]+H.mat[1][i].mat[frame][0]*X.mat[j+1][1];
		}
	}
	return r;
}
...
int main(int argc, char** argv)  {
	...
	char simulationfilename[5000];
	if(argc==2 && strcmp(argv[1], "silent")==0){
		 nb_packet = 250000;
		 optimal_frame_size = 10000;
		 nb_user_min = 20;
		 nb_user_max = 20;
		 nb_user_step = 1;
		 snr_min = 20;
		 snr_max = 20;
		 snr_step = 10;
		 sprintf(simulationfilename, "stbcmultiuser_silentmoderesult_%ld.txt", time(NULL));
		 cout << "Silent mode ... "<<endl;
		 cout << "nb_packet = " << nb_packet << endl;
		 cout << "optimal_frame_size = "<< optimal_frame_size<< endl ;
		 cout << "nb_user_min = "<< nb_user_min << endl ;
		 cout << "nb_user_max = "<< nb_user_max << endl ;
		 cout << "nb_user_step = "<< nb_user_step << endl ;
		 cout << "snr_min = "<< snr_min << endl ;
		 cout << "snr_max = "<< snr_max << endl ;
		 cout << "snr_step = "<< snr_step << endl ;
		 cout << "simulationfilename = " << simulationfilename << endl;
	} else {
	...
	ofstream os(simulationfilename, ios::app);
	os << "**********************************************" << endl;
//	os << "Program compiled on " <<  __TIMESTAMP__ << endl;
	os << __CVS_REVISION__ << endl;
	os << "Run time : "<<time(NULL) <<endl;
	os << "Optimal frame size : "<<optimal_frame_size<<endl;
	os.close();


	double puissance_constellation = 0.5;
	double coef_constellation = sqrt(0.5);
	TConstellation  constellation = psk4gray;
	for(int s=0;s<constellation.size;s++) {
		constellation.sym[s]*=coef_constellation;
	}
	...
	for(int nb_user=nb_user_min;nb_user<=nb_user_max; nb_user+=nb_user_step) {
		for(double SNR=snr_min;SNR<=snr_max;SNR+=snr_step) {
			...
			for(int frame=0;frame<nb_packet<<2;frame+=optimal_frame_size){
				...
				for(Nu=0;Nu<nb_user; Nu++) {
					if(Nu==0) {
						r  =propagateASTBCInFadingChannelWithoutNoise(Xtx.vect[Nu], nb_user, H.vect[Nu], &tapseed);
					} else {
						r +=propagateASTBCInFadingChannelWithoutNoise(Xtx.vect[Nu], nb_user, H.vect[Nu], &tapseed);
					}
				}
				...
				//adding noise
				zRandGausInit(&nptr, 0, convertsnrtosigma2(2*puissance_constellation, SNR));
				ZVector noise(r.vect[0].taille);
				for(Nu=0;Nu<r.taille; Nu++) {
					zbRandGaus(&nptr, noise);
					r.vect[Nu]+=noise;
				}
				// end noise operation
				int htime=0;
			...
			ofstream osres(simulationfilename, ios::app);
			int errorsum;
			errorsum=0;
			for(Nu=0;Nu<nb_user;Nu++) {
...
//	int diff;
	...
	for(int nb_rx=1; nb_rx<=nb_rx_max; nb_rx++) {
		for(double snr=snr_min; snr<=snr_max; snr+=snr_step) {
			...
			WVector bit_out = (*constellation.demodulate_function)(decodeASTBC(propagateASTBCInFadingChannel(X, nb_rx, 1, 1, snr, H, &tapseed), H, constellation));
