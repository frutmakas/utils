...
long_vector codeur_rsc_2(long_vector x, long_vector g, int k, int param)
{
	long_vector a(k,0),/*code,*/x_tail(k-1,0);
	int i, j, accu, L_total;
	int *p_x_tail,*p_code;
...
void backward_treillis_rsc
	(long_vector g, int k, long_matrice &last_state, long_matrice &last_out)
{
	int  m_state, state, i, bit, tmp, m, j,res, a_k;
	m = k-1;
	m_state = 1 << m; /*  2^m  */
	last_state=long_matrice(m_state,2,0);
	last_out=long_matrice(m_state,2,0);
...
void forward_treillis_rsc
		(long_vector g,int k, long_matrice &next_state, long_matrice &next_out)
{
	int m,m_state,i,j,bit,tmp,res,a_k,state;
	m=k-1;
	m_state = 1 << m;
	next_state=long_matrice(m_state,2,0);
	next_out=long_matrice(m_state,2,0);
	for (state=0 ; state<m_state ; state++)
		for (bit=0 ; bit<2 ; bit++){
			tmp = g.vect[0] & state;
			for (i=0, res=0 ; i<m; i++, tmp >>=1)
...
vector sova_paquet(vector sig_rec, int info_len, int n, int k, vector L_a,
			 long_matrice last_state, long_matrice last_out, int ind_dec)
{
...
/* matrice des metriques cumulées initialisée à moins infini */
	matrice m_c(n_state,info_len+1,-infini);


	matrice delta_m(n_state,info_len+1,0);
  long_matrice bit_prec(n_state,info_len+1,0);
	long_vector survivant(info_len+1,0);
	long_vector bit_est(info_len,0);
...
/*----------------------------------------------------------*/
vector matrix_interleaver(vector a,int lin, int col, int param)
{
	int i,j,temp;
	vector out(a.taille,0);
...
/*----------------------------------------------------------*/
long_vector matrix_interleaver(long_vector a,int lin, int col, int param)
{
	int i,j,temp;
	long_vector out(a.taille,0);
	if (param==2) {  /* deinterleaver */
		temp=lin; lin=col; col=temp;}
	for (i=0;i<lin ; i++)
		for (j=0 ; j<col ; j++)
